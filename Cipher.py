...
https://github.com/EasyJailbreak/EasyJailbreak/blob/master/easyjailbreak/attacker/Cipher_Yuan_2023.py
with modifications to make attack work in parallel
...
class Cipher(AttackerBase):
    ...
    def __init__(
        ...
        self.info_dict.update(
            {expert.__class__.__name__: [] for expert in self.mutations}
        )
        self.df = None
    ...
    def single_attack(self, instance: Instance) -> JailbreakDataset:
        ...
        for instance in source_instance_list:
            answer = self.target_model.generate(
                instance.jailbreak_prompt.format(encoded_query=instance.encoded_query),
                max_tokens=1024
            )
        ...
        for i, instance in enumerate(updated_instance_list):
            ...
            instance.target_responses.append(
                mutation.decode(instance.encoded_target_responses)
            )
    ...
    def attack(self):
        ...
        def process_instance(instance):
            ...
            return instance.query, results

        try:
            from utils.parallel import parallel_map

            results = parallel_map(
                process_instance, self.jailbreak_datasets, concurrency=25, use_tqdm=True
            )
            for query, result_set in results:
                self.info_dict["query"].append(query)
    ...
    def update(self, dictionary: dict):
        ...
        self.df["q_s_r"] = self.df.apply(
            lambda row: row[1:].sum() / len(row[1:]), axis=1
        )
        column_probabilities = self.df.iloc[:, 1:].apply(
            lambda col: col.sum() / len(col)
        )
        column_probabilities = pd.Series(
            ["m_s_r"] + list(column_probabilities), index=self.df.columns
        )
    ...
    def log(self):
        ...
        for column in self.df.columns[1:-1]:
            logging.info(
                f"The success rate of {column}:{self.df[column].iloc[-1]* 100:.2f}%"
            )
        logging.info("================Success Rate for Each Item:===============")
        for idx in self.df.index[:-1]:
            query_string = self.df.loc[idx, self.df.columns[0]]
            logging.info(
                f"{idx+1}.The jailbreak success rate of this query is {self.df.loc[idx].iloc[-1]* 100:.2f}%, {query_string}"
            )
