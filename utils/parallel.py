...
def parallel_map(f, *args, concurrency=25, use_tqdm=False, **kwargs):
    ...
    def ensure_iterable(arg):
        return (
            [arg] if not hasattr(arg, "__iter__") or isinstance(arg, str) else list(arg)
        )
    ...
    for i, arg in enumerate(args):
        if len(arg) == 1:
            ...
            raise ValueError(
                f"Argument {i} has length {len(arg)}, expected {max_length}"
            )
    ...
    for k, v in kwargs.items():
        if len(v) == 1:
            ...
            raise ValueError(
                f"Keyword argument '{k}' has length {len(v)}, expected {max_length}"
            )
    ...
    zipped_kwargs = (
        [
            dict(zip(kwargs.keys(), kwarg_values))
            for kwarg_values in zip(*kwargs.values())
        ]
        if kwargs
        else repeat({}, max_length)
    )

    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        futures = [
            executor.submit(f, *a, **kw) for a, kw in zip(zipped_args, zipped_kwargs)
        ]
        if use_tqdm:
